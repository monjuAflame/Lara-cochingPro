    	...
    	<div class="panel-group" id="accordion" style="margin-bottom: 20px;">
    		<div class="panel panel-dafult">
    			...
    			<div class="panel-collapse collapse in" id="collapse1">
    				<div class="panel-body course-detail"><p></p><span style="color: red">{{ Session::get('cmessage') }}</span></div>
    			</div>
    		</div>
    	</div>
    	...
    	<div class="panel panel-dafult">
    		...
    		<div class="panel-body">
    			...
    			{{ csrf_field() }}
    				...
    				<input type="hidden" id="gurdian_id" name="gurdian_id">
    				<input type="hidden" id="school_id" name="school_id">
    				<input type="hidden" id="address_id" name="address_id">
    				<input type="hidden" id="coching_id" name="coching_id" value="{{ sprintf($student_id+1) }}">
    				<input type="hidden" id="user_id" name="user_id" value="{{ Auth::id() }}">
    				<input type="hidden" id="dateregistered" name="dateregistered" value="{{ date('Y-m-d') }}">
    				<input type="hidden" id="s_active" name="s_active" value="1">
								 ...
								 <div class="col-md-4">
									<div class="form-group">
										<label for="coching_id">
											...
											<input type="text" value="{{ $student_id+1 }}" name="coching_id" id="coching_id" class="form-control" readonly>
									</div>
								 </div>
									...
									<tbody>
										<tr>
											<td class="photo">
												...
												<img class="img-responsive student-photo" id="showPhoto" src="{{ asset('img/student.png') }}" alt=" "/>
...
@include('student.popup.gurdianPopup')
@include('student.popup.schoolPopup')
@include('student.popup.addressPopup')
@endsection
...
<script type="text/javascript">
       ...
       // 		e.preventDefault();
       // 		var data = $('#form-create-student').serialize();
       // 		console.log(data);
       // 		$.post("{{route('postInsertStudent')}}" , data, function (data){
       // 			console.log(data);
       ...
       // })
		...
		$(document).on('click','#useCourse',function(e){
			...
			$('.course-detail p').text($(this).text());
		...
		$(document).on('click', '.btn-save-gurdian',function(){
			...
			var data = 'father_name='+fatherName+'&mother_name='+motherName+'&gurdian_name='+gurdianName+'&gurdian_phone='+phone;

            if (fatherName == "" || motherName == "" || gurdianName == "" || phone == "") {
                $('.modal-title span').html('<div style="color:red">require field!!</div>');
            } else {
                $.post("{{ route('postInsertGurdian') }}", data, function(data){
                    console.log(data);
                    $('.modal-title span').html('<div style="color:green">Successfully Inserted!!</div>');
		...
		$(document).on('click', '.btn-save-school',function(){
			...
			var data = 'school_name='+schoolName+'&school_roll='+schoolRoll+'&school_code='+schoolCode;

            if (schoolName == "" || schoolRoll == "" || schoolCode == "") {
                $('.modal-title span').html('<div style="color:red">require field!!</div>');
            } else {
                $.post("{{ route('postInsertSchool') }}", data, function(data){
                    console.log(data);
		...
		$(document).on('click', '.btn-save-address',function(){
			...
			var data = 'village='+village+'&post_office='+postoffice+'&zipcode='+zipcode+'&upazilla='+upazilla+'&district='+district;

            if (village == "" || postoffice == "" || zipcode == "" || upazilla == "" || district == "") {
                $('.modal-title span').html('<div style="color:red">require field!!</div>');
            } else {
                $.post("{{ route('postInsertAddress') }}", data, function(data){
                    console.log(data);
                    $('.modal-title span').html('<div style="color:green">Successfully Inserted!!</div>');
